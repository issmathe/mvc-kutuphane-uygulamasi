...
namespace UdemyProje.Models
{
    ...
    {
        private readonly UygulamaDbContext _uyulamaDbContext;
        internal DbSet<T> dbSet; //dbSet=_uygulamaDbContext karşılık gelir

        public Repository(UygulamaDbContext uygulamaDbContext)
        {
            _uyulamaDbContext = uygulamaDbContext;
            this.dbSet = _uyulamaDbContext.Set<T>();
            _uyulamaDbContext.Kitaplar.Include(k => k.KitapTuru).Include(k => k.KitapTuruId);
        }
